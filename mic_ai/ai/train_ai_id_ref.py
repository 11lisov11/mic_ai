...
def build_env(env_config_path: str, episode_steps: int, w_speed: float, w_power: float, w_smooth: float) -> MicAiAIEnv:
...
def train(env_config: str, episodes: int, episode_steps: int, w_speed: float, w_power: float, w_smooth: float, fast: bool, time_budget_min: float | None) -> Dict[str, str]:
    env = build_env(env_config, episode_steps=episode_steps, w_speed=w_speed, w_power=w_power, w_smooth=w_smooth)
