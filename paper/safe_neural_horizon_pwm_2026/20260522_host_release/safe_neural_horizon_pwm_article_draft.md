...
- The FOC-SVM, FCS-MPC, DTC hysteresis, and DTC-SVM comparisons are now separate host baselines; deadbeat/sensorless rows are still proxy baselines.
- The new FOC-SVM/FCS-MPC/DTC/DTC-SVM baselines are competitive, so SNH-PWM cannot claim classical-control superiority yet.

## Limitations

- Host simulation only.
- FOC-SVM, FCS-MPC, DTC hysteresis, and DTC-SVM are host baselines, but not final tuned publication-grade; deadbeat/sensorless rows are still proxies.
...
- Tune the FOC-SVM/FCS-MPC/DTC/DTC-SVM baselines and replace remaining proxy baselines with tuned strong baselines.
