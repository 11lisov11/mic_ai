...
def _article_draft(payload: Dict[str, Any]) -> str:
    ...
    lines.append("- The FOC-SVM and DTC hysteresis comparisons are now separate host baselines; DTC-SVM/deadbeat/sensorless rows are still proxy baselines.")
    lines.append("- The new FOC-SVM/FCS-MPC/DTC baselines are competitive, so SNH-PWM cannot claim classical-control superiority yet.")
    lines.append("")
    lines.append("## Limitations")
    lines.append("")
    lines.append("- Host simulation only.")
    lines.append("- FOC-SVM, FCS-MPC, and DTC hysteresis are host baselines, but not final tuned publication-grade; DTC-SVM/deadbeat/sensorless rows are still proxies.")
    ...
    lines.append("")
    lines.append("- Tune the FOC-SVM/FCS-MPC/DTC baselines and replace remaining proxy baselines with tuned strong baselines.")
