...
def _article_draft(payload: Dict[str, Any]) -> str:
    ...
    lines.append("- The FOC-SVM, FCS-MPC, DTC hysteresis, DTC-SVM, and deadbeat current comparisons are now separate host baselines; sensorless rows are still proxy baselines.")
    lines.append("- The new FOC-SVM/FCS-MPC/DTC/DTC-SVM/deadbeat baselines are competitive, so SNH-PWM cannot claim classical-control superiority yet.")
    lines.append("")
    lines.append("## Limitations")
    lines.append("")
    lines.append("- Host simulation only.")
    lines.append("- FOC-SVM, FCS-MPC, DTC hysteresis, DTC-SVM, and deadbeat current control are host baselines, but not final tuned publication-grade; sensorless rows are still proxies.")
    ...
    lines.append("")
    lines.append("- Tune the FOC-SVM/FCS-MPC/DTC/DTC-SVM/deadbeat baselines and replace remaining proxy baselines with tuned strong baselines.")
...
def _open_items() -> str:
    return "\n".join(
        [
            ...
            "",
            "- Tune the host key-level FOC-SVM, FCS-MPC, DTC hysteresis, DTC-SVM, and deadbeat baselines and replace remaining host proxy baselines with tuned sensorless/adaptive FOC.",
